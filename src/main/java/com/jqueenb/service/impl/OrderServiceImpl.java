...
import com.jqueenb.dao.*;
import com.jqueenb.pojo.*;
...
import com.jqueenb.vo.CartOrderItemVo;
import com.jqueenb.vo.OrderItemVo;
import com.jqueenb.vo.OrderVo;
import com.jqueenb.vo.ShoppingVo;
...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shoppingId) {
        ...
        Shopping result = shoppingMapper.selectByPrimaryKey(shoppingId);
        if (result == null) {
            return ServerResponse.createByError("没有该收货地址");
        }
    ...
    @Override
    public ServerResponse cancel(Integer userId, Long odrderNo) {
        //stept1:参数非空校验
        if (odrderNo == null) {
            return ServerResponse.createByError("参数不能为空");
        }
        //stept2:根据userid和orderno查询订单
        Order order = orderMapper.findOrderByUseridAndOrderNo(userId, odrderNo);
        if (order == null) {
            return ServerResponse.createByError("订单不存在");
        }
        //stept3:判断订单状态并取消
        if (order.getStatus() != Const.OrderStatusEnum.Order_UN_PAY.getCode()) {
            return ServerResponse.createByError("订单不能被取消");
        }
        //stept5:返回结果
    ...
    public ServerResponse get_order_cart_product(Integer userId) {
        ...
        cartOrderItemVo.setTobalPrice(getOrderPrice(orderItemList));
        //step4:返回结果
        return ServerResponse.createBySuccess(cartOrderItemVo);
    }
    ...
    public ServerResponse list(Integer userId, Integer pageNum, Integer pageSize) {
        ...
        if (orderList == null || orderList.size() == 0) {
            return ServerResponse.createByError("未找到订单信息");
        }
...
//        3判断订单状态并发货和更改发货时间
        if (order.getStatus() != Const.OrderStatusEnum.Order_PAYDE.getCode()) {//必须是已付款
            return ServerResponse.createByError("该订单不能发货");
        }
...
//        4返回值
    ...
    private ServerResponse getCartOrderItem(Integer userId, List<Cart> cartList) {
        ...
        for (Cart cart : cartList) {
            ...
            if (product == null) {
                return ServerResponse.createByError("id为" + cart.getProductId() + "的商品不存在");
            }
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        //step1:获取订单号
        Long out_trade_no=Long.parseLong(map.get("out_trade_no"));
        //step2:获取流水号
        ...
        Order order=orderMapper.findOrderByOrderNo(out_trade_no);
        if(order==null){
            return ServerResponse.createByError("订单"+out_trade_no+"不是本商品的订单号");
        }
        if(order.getStatus()>=Const.OrderStatusEnum.Order_PAYDE.getCode()){
            //防止重复调用
        ...
        if(trade_status.equals(Const.TRADE_SUCCESS)){
            //支付成功
            //更改订单的状态，更改支付时间
            order.setStatus(Const.OrderStatusEnum.Order_PAYDE.getCode());
        ...
        return ServerResponse.createByError("致富信息保存失败");
    }
    ...
    public ServerResponse pay(Order order) throws IOException {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://2uzfwk.natappfree.cc/trfshopback/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
